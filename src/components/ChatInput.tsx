...
export const ChatInput: React.FC<ChatInputProps> = ({ placeholder, onSend }) => {
  ...
  const onInputTextMouseDown = useCallback((event: React.KeyboardEvent<HTMLInputElement>) => { 
    if (event.ctrlKey && event.key === 'Enter') { 
      sendMessage(inputText)
    }
  }, [sendMessage, inputText])

  const handleInputClick = useCallback((event: MouseEvent<HTMLAnchorElement>) => {
    sendMessage(inputText)
  }, [sendMessage, inputText]);
